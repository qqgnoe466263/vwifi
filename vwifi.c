...
{
    struct owl_vif *owl = ndev_get_owl_vif(ndev);

    pr_info("owl: start ap\n");
    ...
    /* set AP SSID */
    memcpy(owl->ssid, settings->ssid, settings->ssid_len);
    update_ssids(owl->ssid);
        ...
        if (!strncmp(ap->bssid, vif->bssid, ETH_ALEN)) {
            ...
            list_for_each_entry (item, &owl->vif_list, list) {
                if (!strncmp(vif->bssid, item->bssid, ETH_ALEN) &&
                    ...
                    /* Disconnect STA if some STA connected AP */
